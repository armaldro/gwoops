    ...
    <div className="grid gap-8 lg:grid-cols-[minmax(0,420px)_1fr]">
      ...
      <div className="min-w-0 space-y-6">
        <header>
          ...
          <h1 className="mt-2 font-display text-2xl sm:text-3xl">{item.name}</h1>
            ...
            <dl className="card divide-y text-sm">
              {filled.map((field) => (
                <div key={field.key} className="flex gap-4 px-4 py-2.5">
                  <dt className="w-36 shrink-0" style={{ color: 'var(--ink-muted)' }}>
                    {field.label}
                  </dt>
        ...
        {(item.purchase_price || item.est_value || item.warranty_ends_at) && (
          <section>
            ...
            <dl className="card divide-y text-sm">
              ...
              {item.purchase_date && (
                <Row label="Bought">{formatDate(item.purchase_date)}</Row>
              )}
            </dl>
          </section>
        )}
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-2">
              {bundleLinks.map((link) => {
                const bundle = (link as unknown as {
